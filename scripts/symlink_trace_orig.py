...
if not os.path.exists(trace_orig):
    cprint(scriptfile +
           "The directory for 'trace_orig' as defined in options.yaml does",
...
if os.path.islink("trace_orig"):
    ...
    cprint(scriptfile + "'trace_orig' is an existing subdirectory.", "red")
    ...
    cprint(scriptfile + "'trace_orig' is an existing file.", "red")
    cprint("I cannot create a symbolic link there. Please remove the "
...
cprint(scriptfile +
       "Creating symbolic link from '%s' to subdirectory 'trace_orig'."
       % trace_orig, "green")
