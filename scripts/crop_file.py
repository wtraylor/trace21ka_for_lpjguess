...
if len(sys.argv) != 3:
    cprint(scriptfile + "Please provide exactly two command line arguments.",
           "red")
    sys.exit(1)
...
if not os.path.isfile(in_file):
    cprint(scriptfile + "Input file does not exist.", "red")
    sys.exit(1)
...
cprint(scriptfile + "'%s'..." % in_file, "green")
...
if status != 0:
    cprint(scriptfile + "Cropping with `ncks` failed: Bad return code.",
           "red")
...
if not os.path.isfile(out_file):
    cprint(scriptfile + "Cropping with `ncks` failed: No output file "
           "created.", "red")
    cprint("Input file:", in_file, "red")
    cprint("Output file:", out_file, "red")
    sys.exit(1)
